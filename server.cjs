...
const server = http.createServer(async (req, res) => {
  ...
  // GET /api/lb-release - KiraBoard version check
  if (req.method === 'GET' && pathname === '/api/lb-release') {
    ...
    (async () => {
      try {
        ...
        const ghRes = await fetch('https://api.github.com/repos/CoderMariusz/kira-dashboard/releases/latest');
        const ghData = await ghRes.json();
        const result = {
          status: 'ok',
          current: currentVersion,
  ...
  if (req.method === 'GET' && pathname === '/api/rss') {
    ...
    try {
      ...
      function fetchFeed(url, redirects) {
        ...
        const req2 = mod.get(url, { headers: { 'User-Agent': 'KiraBoard/1.0' }, timeout: 15000 }, (proxyRes) => {
  ...
  if (req.method === 'GET' && pathname === '/api/quote') {
    const https = require('https');
    https.get('https://zenquotes.io/api/random', { headers: { 'User-Agent': 'KiraBoard/1.0' }, timeout: 5000 }, (proxyRes) => {
      let body = '';
...
server.listen(PORT, HOST, () => {
  console.log(`
🦊 KiraBoard Builder Server running at http://${HOST}:${PORT}
