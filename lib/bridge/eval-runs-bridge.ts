 ...
 *
 * Reads from Bridge SQLite DB using a Python subprocess (same pattern as eval-tasks-bridge.ts).
 */
...
/**
 * Return paginated eval run history from Bridge DB.
 */
...
/**
 * Return a single eval run by ID, or null if not found.
 */
...
/**
 * Return task results for a run, plus the previous run ID + its task results
 * (needed for diff computation in the route).
 */
...
# Current run task results — join eval_tasks for prompt + category
cur.execute("""
  SELECT
    tr.id, tr.run_id, tr.task_id,
    tr.actual_output,
    tr.passed,
    COALESCE(tr.diff_score, 0.0) AS diff_score,
    tr.created_at,
    SUBSTR(COALESCE(et.prompt, ''), 1, 200) AS task_prompt,
    et.category AS task_category
  FROM eval_run_task_results tr
  LEFT JOIN eval_tasks et ON et.id = tr.task_id
  WHERE tr.run_id = ?
""", (${JSON.stringify(safeRunId)},))
...
cur.execute("""
  ...
  WHERE started_at < (SELECT started_at FROM eval_runs WHERE id = ?)
...
print(json.dumps({
  ...
  "prevTaskResults": prev_task_results,
}))
`
